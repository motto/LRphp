...
if($adatok['password']!=''){$uj_jelszo=md5(ELL::$adatok['password']);}else{$uj_jelszo=$jelszo;}
	...
	include 'login'.DS.'wiev'.DS.'belep_form.php';
	}else{
	Tomb::kiir(GOB::$hiba['ELL']);
	
	include 'login'.DS.'wiev'.DS.'valtoztat_form.php';
	}
...
switch ($_GET['login']){
	case 'reg':
	include 'login'.DS.'wiev'.DS.'regisztral_form.php';
		break;
	case 'valtoztat':
	include 'login'.DS.'wiev'.DS.'valtoztat_form.php';
		break;
	...
	if($_SESSION['userid']>0)
		{include 'login'.DS.'wiev'.DS.'belepve_form.php';}
		else
		{include'login'.DS.'wiev'.DS.'belep_form.php';}	
	}
